...
class DNSPacket:
	...
	func _init(packet : StreamPeerBuffer) -> void:
		...
		for i in range(question_length):
			dns_questions.append(DNSQuestion.new(packet))
		for i in range(question_length):
			dns_answers.append(DNSRecord.new(packet))
		for i in range(question_length):
			dns_authoritories.append(DNSRecord.new(packet))
		for i in range(question_length):
			dns_additional.append(DNSRecord.new(packet))
...
class DNSQuestion:
	...
	func _read_labels(packet : StreamPeerBuffer) -> Array[String]:
		...
		if length == 0:
			return inner_labels
		var raw_data = packet.get_data(length)
...
class DNSRecord:
	...
	func _txt_record(packet : StreamPeerBuffer) -> void:
		...
		while l > 0:
			...
			var part : PackedByteArray = PackedByteArray(packet.get_data(part_length))
	...
	func _get_ipv4_address(packet : StreamPeerBuffer) -> String:
		...
		return "%s.%s.%s.%s" % [ip_bytes[0], ip_bytes[1], ip_bytes[2], ip_bytes[3]]
...
func _ready() -> void:
	...
	var err = receiver.bind(5353, "0.0.0.0")
